...
const ALL_PERFUMES = Array.from({ length: 20 }, (_, i) => ({
  ...
  image: `https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?q=80&w=500&sig=${i}`
}));
...
const Collections = () => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen bg-white pt-24 pb-20" // Reduced from pt-32 to raise content
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex justify-center space-x-12 mb-12 border-b border-neutral-100 pb-6">
          {categories.map((cat) => (
            ...
            >
              <span className={`text-[11px] uppercase tracking-[0.3em] font-bold transition-colors duration-300 ${
                filter === cat.name ? 'text-red-700' : 'text-neutral-400 group-hover:text-black'
              }`}>
                {cat.name === "All" ? cat.name : `For ${cat.name}`}
              </span>
              <span className="text-[9px] text-neutral-300 mt-1">{cat.count} Scent(s)</span>
        ...
        {/* Product Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12"> {/* Reduced gap-y */}
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((product, index) => (
              <motion.div
                key={product.id}
                layout
                initial={{ opacity: 0, y: 20 }} // Changed scale to y-offset for smoother entry
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95 }}
                transition={{ 
                  duration: 0.5, 
                  delay: index * 0.03, // Faster stagger delay
        ...
        {/* Pagination Section */}
        <div className="mt-24 flex flex-col items-center"> {/* Reduced margin-top */}
