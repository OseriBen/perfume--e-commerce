...
const ProductDetail = () => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen bg-[#FBFBFB] pt-24"
    >
      <div className="max-w-[1440px] mx-auto px-6">
        
        {/* Back Button */}
        <Link to="/collections" className="inline-flex items-center text-[10px] uppercase tracking-[0.3em] text-neutral-400 hover:text-black transition-colors mb-12">
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          {/* Right: Content & Scent Notes */}
          <div className="lg:w-1/2 pb-20">
            <motion.div variants={fadeUp} initial="hidden" animate="visible">
              <span className="text-red-700 text-[10px] uppercase tracking-[0.5em] font-bold">№ {id} Exclusive</span>
              <h1 className="text-5xl md:text-6xl font-serif mt-4 mb-6 italic tracking-tight text-black">
                {productData.name.split(' ').slice(0, 2).join(' ')} <br /> {productData.name.split(' ').slice(2).join(' ')}
              </h1>
              <p className="text-2xl font-serif text-neutral-800 mb-10">${productData.price.toFixed(2)}</p>
              
              <div className="h-[1px] w-full bg-neutral-200 mb-10"></div>

              <p className="text-neutral-500 leading-relaxed font-light mb-12 text-lg">
                An olfactory journey through the ancient forests of the East. Oud Mystique captures the essence of midnight air—woody, smoky, and dangerously elegant.
              </p>

              {/* Scent Architecture */}
              <div className="space-y-10 mb-16">
                <h3 className="text-[11px] uppercase tracking-[0.4em] font-bold text-black border-b border-black/10 pb-4">The Scent Architecture</h3>
